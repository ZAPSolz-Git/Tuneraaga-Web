...
export default function MyArtists() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (session?.user) {
        ...
        await fetchFollowedArtists(session.user.id);
      } else {
        setLoading(false);
      }
    };
  ...
  const fetchFollowedArtists = async (uid) => {
    setLoading(true);
    try {
      ...
      const artistDetails = await Promise.all(
        data.map(async (entry) => {
          const { data: releases } = await supabase
            .from("releases")
            .select(
              "cover_url, primary_artist, title, audio_url, duration, id, featuring_artists, album_name",
            )
            .eq("primary_artist", entry.artist_name)
            .eq("status", "Published")
            .limit(20);
          const firstRelease = releases?.[0] || null;
          const albumSet = new Set(
            releases?.map((r) => r.album_name).filter(Boolean) || [],
          );
          return {
            ...entry,
            cover_url: firstRelease?.cover_url || null,
            song_count: releases?.length || 0,
            album_count: albumSet.size,
            songs: releases || [],
          };
        }),
      );
      setFollowedArtists(artistDetails);
    } catch (err) {
      setFollowedArtists([]);
    }
    setLoading(false);
  ...
  const unfollowArtist = async (artistId) => {
    ...
    const { error } = await supabase
      ...
      .delete()
      .eq("id", artistId);
    if (!error) setFollowedArtists((p) => p.filter((a) => a.id !== artistId));
    setRemovingId(null);
  };

  const clearAllArtists = async () => {
    if (!user) return;
    handleClosePlayer();
    const { error } = await supabase
      .from("followed_artists")
  ...
  const playArtist = (artist) => {
    ...
    if (first.audio_url) playSong(first, artist.songs, 0);
  };
  ...
  const TabBar = () => (
    <div className="flex items-center gap-1 bg-white/10 rounded-xl p-1 overflow-x-auto">
      ...
      <Link
        to="/my-artists"
        className="px-4 py-2 rounded-lg text-sm font-semibold transition-all bg-white text-emerald-700 shadow-sm whitespace-nowrap"
      >
  ...
  if (!user && !loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-50 via-emerald-50/40 to-slate-50 p-4">
        <div className="relative bg-white/80 backdrop-blur-xl border border-slate-200 rounded-3xl shadow-xl shadow-emerald-100/50 px-8 py-12 md:px-14 md:py-16 max-w-md w-full text-center overflow-hidden">
          {/* Decorative blurred circles */}
          <div className="absolute -top-10 -right-10 w-32 h-32 bg-emerald-200/40 rounded-full blur-2xl" />
          <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-teal-200/40 rounded-full blur-2xl" />

          {/* Icon */}
          <div className="relative w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-200">
          ...
          <p className="relative text-slate-500 mb-8 text-sm md:text-base leading-relaxed">
            Please login to view your followed artists and stay updated on their
            latest releases.
          </p>

          <button
            onClick={() => setShowAuthModal(true)}
            className="relative w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-8 py-3.5 rounded-full font-bold shadow-lg shadow-emerald-200 hover:shadow-xl hover:shadow-emerald-300 hover:scale-105 active:scale-95 transition-all duration-300"
          >
            Login / Sign Up
          </button>
        </div>
  ...
  return (
    <div className="w-full min-h-screen text-slate-900 pb-32 relative">
      {showAuthModal && <Auth onClose={() => setShowAuthModal(false)} />}
      <div className="bg-gradient-to-br from-emerald-700 via-teal-800 to-cyan-900 text-white px-4 md:px-8 pt-8 pb-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-extrabold flex items-center gap-2">
              <Users size={22} className="text-emerald-500" /> Artists
            </h2>
            <p className="text-slate-500 text-sm mt-1">
              {followedArtists.length} artist
              {followedArtists.length !== 1 ? "s" : ""}
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="flex items-center justify-center gap-3 flex-wrap">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-emerald-600 text-white font-semibold text-sm hover:bg-emerald-700 shadow-md"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {followedArtists.map((artist) => (
              <div
                key={artist.id}
                className={`group bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all overflow-hidden ${removingId === artist.id ? "opacity-40 pointer-events-none" : ""}`}
              >
                <div className="relative flex flex-col items-center pt-6 pb-3">
                  <div className="relative">
                    <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-lg">
                      ...
                      <div
                        className="w-full h-full bg-gradient-to-br from-emerald-400 to-teal-600 items-center justify-center text-white text-3xl font-bold"
                    ...
                    <button
                      ...
                      className="absolute bottom-0 right-0 w-10 h-10 rounded-full bg-emerald-600 text-white flex items-center justify-center shadow-lg opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 hover:bg-emerald-700 hover:scale-110"
                    >
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 w-7 h-7 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-red-100 hover:text-red-600"
                  >
                ...
                <div
                  className="px-3 pb-4 text-center cursor-pointer"
                  onClick={() =>
                    navigate(
