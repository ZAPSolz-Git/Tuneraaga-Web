...
export default function MyPodcasts() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (session?.user) {
        ...
        await fetchSavedPodcasts(session.user.id);
      } else {
        setLoading(false);
      }
    };
  ...
  const fetchSavedPodcasts = async (uid) => {
    setLoading(true);
    try {
      const { data, error } = await supabase
        .from("saved_podcasts")
        .select("id, podcast_name, created_at")
        .eq("user_id", uid)
      ...
      const podcastDetails = await Promise.all(
        data.map(async (entry) => {
          const { data: releases } = await supabase
            ...
            .eq("album_name", entry.podcast_name)
            .eq("status", "Published")
            .limit(20);
          ...
          return {
            ...
            episode_count: releases?.length || 0,
            episodes: releases || [],
          };
        }),
      );
      setSavedPodcasts(podcastDetails);
    } catch (err) {
      setSavedPodcasts([]);
    }
    setLoading(false);
  ...
  const removePodcast = async (podcastId) => {
    ...
    const { error } = await supabase
      ...
      .delete()
      .eq("id", podcastId);
    if (!error) setSavedPodcasts((p) => p.filter((a) => a.id !== podcastId));
    setRemovingId(null);
  };

  const clearAllPodcasts = async () => {
    if (!user) return;
    handleClosePlayer();
    const { error } = await supabase
      .from("saved_podcasts")
  ...
  const playPodcast = (pod) => {
    if (!pod.episodes || pod.episodes.length === 0) return;
    const first = pod.episodes[0];
    if (first.audio_url) playSong(first, pod.episodes, 0);
  };
  ...
  const TabBar = () => (
    <div className="flex items-center gap-1 bg-white/10 rounded-xl p-1 overflow-x-auto">
      ...
      <Link
        to="/my-podcasts"
        className="px-4 py-2 rounded-lg text-sm font-semibold transition-all bg-white text-orange-700 shadow-sm whitespace-nowrap"
      >
        <Podcast size={14} className="inline mr-1.5" />
        Podcasts
      </Link>
      <Link
        to="/my-artists"
  ...
  if (!user && !loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-4">
        <Mic2 size={48} className="text-slate-300 mb-4" />
        <h2 className="text-2xl font-bold mb-2">Login Required</h2>
        <p className="text-slate-500 mb-6">
          Please login to view your saved podcasts.
        </p>
        <button
  ...
  return (
    <div className="w-full min-h-screen text-slate-900 pb-32 relative">
      {showAuthModal && <Auth onClose={() => setShowAuthModal(false)} />}
      <div className="bg-gradient-to-br from-orange-600 via-amber-700 to-red-800 text-white px-4 md:px-8 pt-8 pb-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-extrabold flex items-center gap-2">
              <Mic2 size={22} className="text-orange-500" /> Podcasts
            </h2>
            <p className="text-slate-500 text-sm mt-1">
              {savedPodcasts.length} podcast
              {savedPodcasts.length !== 1 ? "s" : ""}
            </p>
          </div>
          {savedPodcasts.length > 0 && (
            <button
              onClick={clearAllPodcasts}
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 shadow-sm">
            <div className="w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center mx-auto mb-5">
              <Mic2 size={36} className="text-slate-300" />
            </div>
            ...
            <div className="flex items-center justify-center gap-3 flex-wrap">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-orange-600 text-white font-semibold text-sm hover:bg-orange-700 shadow-md"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {savedPodcasts.map((pod) => (
              <div
                key={pod.id}
                className={`group bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all overflow-hidden ${removingId === pod.id ? "opacity-40 pointer-events-none" : ""}`}
              >
                <div className="relative aspect-square overflow-hidden">
                  <img
                    src={pod.cover_url || "https://via.placeholder.com/200"}
                    alt={pod.podcast_name}
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                    <button
                      ...
                      className="w-12 h-12 rounded-full bg-orange-600 text-white flex items-center justify-center shadow-lg opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 hover:bg-orange-700 hover:scale-110"
                    >
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      removePodcast(pod.id);
                    }}
                    disabled={removingId === pod.id}
                ...
                <div
                  ...
                  onClick={() =>
                    navigate(`/album/${encodeURIComponent(pod.podcast_name)}`)
                  }
                >
                  ...
                  <p className="text-[11px] text-orange-500 mt-1">
                    {pod.episode_count} episode
                    {pod.episode_count !== 1 ? "s" : ""}
                  </p>
                </div>
              </div>
            ))}
