...
const TopPlaylistAdmin = () => {
  ...
  const nextStep = () => {
    ...
    if (step === 2 && selectedReleaseIds.length === 0)
      return alert("Please select at least one song from your library.");
  ...
  const handleSubmit = async () => {
    if (!formData.copyrightHolder)
      return alert("Copyright Holder is required.");
    setLoading(true);
    try {
      let finalImageUrl = "";
      ...
      for (const releaseId of selectedReleaseIds) {
        ...
        if (releaseData) {
          await supabase.from("playlist_songs").insert({
            ...
            featuring_artists: releaseData.featuring_artists || null,
            album_name: releaseData.album_name || null,
            cover_url: releaseData.cover_url || null,
  ...
  const handleAddReleaseToExisting = async (releaseId) => {
    ...
    const alreadyExists = activePlaylistForManage.playlist_songs?.some(
      (s) => s.release_id === releaseId,
    );
    if (alreadyExists) {
    ...
    try {
      ...
      if (updatedListData) {
        ...
        setPlaylists((prev) =>
          prev.map((p) => (p.id === updatedListData.id ? updatedListData : p)),
        );
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="p-4 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            {view === "wizard"
              ? "Create a new playlist using your release library."
              : "View and manage movie playlists."}
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden min-h-[600px] flex flex-col">
              ...
              {steps.map((s, i) => (
                <div key={s.id} className="flex items-center flex-1">
                  <div className="flex flex-col items-center gap-1 w-full">
                    <div
                      className={`h-8 w-8 rounded-full flex items-center justify-center border-2 transition-all ${step >= i ? "border-blue-600 bg-blue-600 text-white" : "border-gray-300 text-gray-400 bg-white"}`}
                    >
                      {step > i ? (
                    ...
                    <span
                      className={`text-[10px] font-bold tracking-wider uppercase ${step >= i ? "text-blue-700" : "text-gray-400"}`}
                    >
                      {s.label}
                    </span>
                  </div>
                  {i < steps.length - 1 && (
                    <div
                      className={`h-0.5 w-full mx-2 ${step > i ? "bg-blue-600" : "bg-gray-200"}`}
                    ></div>
                  )}
                </div>
              ))}
            ...
            {step === 0 && (
              <div className="max-w-3xl mx-auto text-center animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <p className="text-gray-500 text-sm mb-6">
                  This image will represent your playlist.
                </p>
                <div
              ...
              <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea
                      rows="3"
                      className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:border-blue-500 outline-none resize-none"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[500px]">
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-3 overflow-hidden">
                                <span className="text-slate-400 font-bold w-4 shrink-0">
                                  {idx + 1}
                                </span>
                                {song.cover_url ? (
                                  <img
                              ...
                              <button
                                onClick={() => toggleReleaseSelection(id)}
                  ...
                  <div className="bg-white p-6 rounded-xl border border-slate-200 flex flex-col h-full">
                    ...
                    <div className="flex-grow overflow-y-auto space-y-2 pr-1">
                      ...
                      {getFilteredReleases(releaseSearchQuery).map(
                        (release) => {
                          const isSelected = selectedReleaseIds.includes(
                            release.id,
                          );
                          return (
                            <div
                                ...
                                <div className="min-w-0">
                                  ...
                                  <p className="text-xs text-slate-500 truncate">
                                    ...
                                    {release.featuring_artists
                                      ? ` ft. ${release.featuring_artists}`
                                      : ""}
                                  </p>
                                </div>
                              </div>
                              <button
                                className={`p-1.5 rounded-full ${isSelected ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-400"}`}
                              >
                                {isSelected ? (
                                  <CheckCircle2 size={16} />
                                ) : (
                ...
                <div className="mt-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-yellow-700">
                      This will create the playlist "{formData.title}" with{" "}
                      {selectedReleaseIds.length} songs linked from your Release
                      Library.
                    </p>
                  </div>
                </div>
          ...
          <div className="border-t border-slate-200 bg-slate-50 p-4 md:px-8 flex justify-between items-center">
            <button
              onClick={prevStep}
              disabled={step === 0}
              className="px-6 py-2 rounded-lg border border-slate-300 text-slate-600 font-bold hover:bg-white disabled:opacity-50 transition"
            >
              Previous
            </button>
            {step < 3 ? (
              ...
              <button
                ...
                disabled={loading}
                className="px-6 py-2 rounded-lg bg-green-600 text-white font-bold shadow hover:bg-green-700 transition flex items-center gap-2 disabled:opacity-50"
              >
                {loading ? (
              ...
              {viewMode === "grid" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 pb-20">
                  {playlists.map((playlist) => (
                    <div
                      key={playlist.id}
                      className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-all group relative flex flex-col"
                    >
                      ...
                      <div className="p-4 flex flex-col flex-grow">
                        ...
                        <button
                          ...
                          className="w-full py-2 bg-blue-50 text-blue-600 text-xs font-bold rounded-lg hover:bg-blue-600 hover:text-white transition-colors flex items-center justify-center gap-2 mt-auto"
                        >
                  ...
                  {playlists.length === 0 && (
                    <div className="col-span-full text-center py-20 text-slate-400 flex flex-col items-center">
                      ...
                      <p className="text-lg">
                        No playlists found. Click "Create New" to start.
                      </p>
                    </div>
                  )}
                </div>
      ...
      {view === "list" && (
        <button
          ...
          className="fixed bottom-8 right-8 z-50 w-14 h-14 rounded-full text-white flex items-center justify-center shadow-lg hover:scale-110 transition-transform"
              ...
              <div className="flex-grow overflow-y-auto p-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
                  <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 h-fit flex flex-col">
                    ...
                    <div className="relative mb-4">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-4 py-2 border border-blue-200 bg-white rounded-lg text-sm focus:border-blue-500 outline-none"
                        value={manageSearchQuery}
                        onChange={(e) => setManageSearchQuery(e.target.value)}
                      />
                    </div>
                    <div className="flex-grow overflow-y-auto space-y-2 max-h-[400px]">
                      {getFilteredReleases(manageSearchQuery).length === 0 && (
                        <div className="text-center text-blue-300 py-8 text-sm">
                          No releases found.
                        </div>
                      )}
                      {getFilteredReleases(manageSearchQuery).map((release) => {
                        const isAdded =
                          activePlaylistForManage.playlist_songs?.some(
                            (s) => s.release_id === release.id,
                          );
                        return (
                          <div
                            key={release.id}
                            className={`flex items-center justify-between p-3 bg-white border rounded-lg ${isAdded ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:border-blue-500"}`}
                          >
                            <div className="flex items-center gap-3 overflow-hidden">
                              <img
                                src={
                            ...
                            <button
                              disabled={isAdded}
                              onClick={() =>
                                handleAddReleaseToExisting(release.id)
                              }
                              className={`p-1.5 rounded-full ${isAdded ? "bg-slate-100 text-slate-400" : "bg-blue-600 text-white hover:bg-blue-700"}`}
                            >
                              {isAdded ? (
                                <CheckCircle2 size={16} />
                              ) : (
                  ...
                  <div className="flex flex-col h-full">
                    <h4 className="font-bold text-gray-900 mb-4">
                      Current Tracklist (
                      {activePlaylistForManage.playlist_songs?.length || 0})
                    </h4>
                    <div className="flex-grow overflow-y-auto">
                      {!activePlaylistForManage.playlist_songs ||
                      activePlaylistForManage.playlist_songs.length === 0 ? (
                        <div className="p-10 border-2 border-dashed border-slate-200 rounded-xl text-center text-slate-400 h-full flex items-center justify-center">
                          This playlist is empty. Add songs from your library on
                          the left.
                        </div>
                      ) : (
                        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                          {activePlaylistForManage.playlist_songs.map(
                            (song, idx) => (
                              <div
                                key={song.id}
                              ...
                              >
                                ...
                                <button
                                  onClick={() =>
                                    handleDeleteSongFromExisting(song.id)
                                  }
