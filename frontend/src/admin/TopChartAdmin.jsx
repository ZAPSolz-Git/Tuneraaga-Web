...
const TopChartAdmin = () => {
  ...
  const handleSubmit = async () => {
    if (!formData.copyright_holder)
      return Swal.fire("Required", "Copyright Holder is required.", "warning");
    setLoading(true);
    try {
      let finalImageUrl = "";
  ...
  const nextStep = () => {
    if (step === 1 && !formData.image)
      return Swal.fire("Required", "Please upload a cover.", "warning");
    if (step === 2) {
      if (!formData.title)
        return Swal.fire("Required", "Chart Title is required.", "warning");
      if (!formData.artist)
        return Swal.fire("Required", "Artist Name is required.", "warning");
      if (!formData.genre)
        return Swal.fire("Required", "Genre is required.", "warning");
    }
    if (step === 3 && songs.length === 0)
      return Swal.fire("Required", "Please add at least one song.", "warning");
                ...
                <div className="mt-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-yellow-700">
                      Ensure you have the rights to publish "{formData.title}"
                    </p>
                  </div>
                </div>
              </div>
            ...
            {step === 5 && (
              <div className="max-w-2xl mx-auto text-center py-10">
                ...
                <p className="text-slate-500 mb-6">
                  Artist: {formData.artist} | Genre: {formData.genre} |{" "}
                  {songs.length} Songs
                </p>
                <button
            ...
            >
              ...
              <div className="flex-grow overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 h-fit">
                  ...
                  <form
                    onSubmit={handleAddSongToExisting}
                    className="space-y-3"
                  >
                    <div>
            ...
            >
              <div className="bg-slate-50 border-b border-slate-200 p-6 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-slate-500">{editingChart.title}</p>
                </div>
                ...
                <form onSubmit={handleUpdateChart} className="space-y-4">
                  ...
                  <div className="mt-6 flex justify-end gap-3 pt-4 border-t border-slate-100">
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="px-6 py-2 bg-blue-600 text-white rounded-lg font-bold shadow hover:bg-blue-700 flex items-center gap-2"
                    >
                      {loading ? (
